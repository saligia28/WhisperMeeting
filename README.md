...
- **🎙️ WebSocket 实时转写**：基于 PCM 音频流的真正实时语音识别，延迟 2-3 秒，无浏览器兼容性问题
- **🚀 FastAPI 后端**：提供录音、转写、摘要、下载等完整 RESTful 接口
- **📦 批量转写**：支持上传完整音频文件进行离线处理 (`/meetings/{id}/transcribe`)
- **💻 CLI 工具**：可对单个音频文件执行"片段 → 转写 → 摘要"的串联流程
- **🔧 模块化设计**：涵盖音频采集、转写、说话人分离、摘要、存储等组件，便于替换具体实现
- **📊 硬件评估**：提供硬件评估脚本和测试计划，帮助验证目标环境的可行性
...
使用基于 WebSocket + PCM 的实时转写，无浏览器兼容性问题：

1. 启动后端服务（见上文）
2. 在浏览器中打开 `examples/realtime_transcription.html`
3. 点击"开始录音"即可看到实时转写结果

**技术特点：**
- ✅ 使用 AudioContext API 采集原始 PCM 音频流，绕过浏览器编码差异
- ✅ 通过 WebSocket 实时传输，延迟 2-3 秒
- ✅ 完全基于本地 Whisper 模型，数据不出服务器
...
- 或通过 API 上传完整音频文件：
  ...
  curl -X POST http://localhost:8000/meetings \
    ...
    -d '{"meeting_id": "test123", "title": "测试会议"}'

  # 上传音频进行转写
  curl -X POST http://localhost:8000/meetings/test123/transcribe \
    -F "audio=@your_audio.wav"

  # 查询转写状态
  curl http://localhost:8000/meetings/test123/status

  # 获取转写结果
  curl http://localhost:8000/meetings/test123/result
  ```
...
- ✅ **实时转写已实现**：基于 WebSocket + PCM 的实时语音识别已完成，延迟 2-3 秒
